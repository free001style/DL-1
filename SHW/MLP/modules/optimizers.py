...
class SGD(Optimizer):
    ...
    def step(self):
        ...
        for param, grad, m in zip(parameters, gradients, self.state['m']):
            ...
            """
            np.add(self.weight_decay * param, grad, out=grad)
            np.add(np.multiply(self.momentum, m, out=m), grad, out=m)
...
class Adam(Optimizer):
    ...
    def step(self):
        ...
        for param, grad, m, v in zip(parameters, gradients, self.state['m'], self.state['v']):
            ...
            """
            np.add(self.weight_decay * param, grad, out=grad)
            np.add(np.multiply(self.beta1, m, out=m), (1 - self.beta1) * grad, out=m)
            np.add(np.multiply(self.beta2, v, out=v), (1 - self.beta2) * np.square(grad), out=v)
